...
class Communicator(object):
    ...
    def _apply_reduce_op(self, acc, other, op):
        if op == MPI.SUM:
    ...
    def myAllreduce(self, src_array, dest_array, op=MPI.SUM):
        ...
        sendbuf = np.ascontiguousarray(src_array)
        nbytes = sendbuf.itemsize * sendbuf.size

        if rank == root:
            result = np.array(sendbuf, copy=True)
            recvbuf = np.empty_like(result)
            for src in range(1, nprocs):
                self.comm.Recv(recvbuf, source=src)
                self.total_bytes_transferred += nbytes
                self._apply_reduce_op(result, recvbuf, op)
            np.copyto(dest_array, result)
            result = np.ascontiguousarray(dest_array)
            for dest in range(1, nprocs):
                self.comm.Send(result, dest=dest)
                self.total_bytes_transferred += nbytes
        else:
            self.comm.Send(sendbuf, dest=root)
            self.total_bytes_transferred += nbytes
            recvbuf = np.empty_like(sendbuf)
            self.comm.Recv(recvbuf, source=root)
            self.total_bytes_transferred += nbytes
            np.copyto(dest_array, recvbuf)
    ...
    def myAlltoall(self, src_array, dest_array):
        ...
        send = np.ascontiguousarray(src_array).ravel()
        recv = np.ascontiguousarray(dest_array).ravel()
        seg = send.size // nprocs
        seg_bytes = send.itemsize * seg

        # Copy the local segment without a network round-trip.
        recv[rank * seg : (rank + 1) * seg] = send[rank * seg : (rank + 1) * seg]

        # Rotate Sendrecv partners so each offset is a matching pairwise exchange.
        for offset in range(1, nprocs):
            dest = (rank + offset) % nprocs
            source = (rank - offset) % nprocs
            sendbuf = send[dest * seg : (dest + 1) * seg]
            recvbuf = recv[source * seg : (source + 1) * seg]
            self.comm.Sendrecv(sendbuf, dest=dest, recvbuf=recvbuf, source=source)
            self.total_bytes_transferred += 2 * seg_bytes

        if recv.base is not dest_array and recv is not dest_array.ravel():
            np.copyto(dest_array.ravel(), recv)
