...
class ShardedLinear:
    ...
    def __call__(self, x):
        # Handle empty batch case. Still participate in the all-reduce so that
        # ranks cannot deadlock if some processes receive no tokens.
        if x.shape[0] == 0:
            local_out = np.zeros((0, self.local_out_features), dtype=self.weight.dtype)
        else:
            local_out = np.dot(x, self.weight) + self.bias

        # Column-parallel: each rank owns a contiguous slice of the output.
        # Write the local slice and all-reduce so every rank sees the full vector.
        result = np.zeros((x.shape[0], self.out_features_global), dtype=local_out.dtype)
...
class MoE_TP:
    ...
    def forward(self, x):
        ...
        outputs = np.zeros((batch_size, self.output_dim), dtype=x.dtype)

        # 1. Compute the routing indices and gates for each input
        indices, gates = self.router(x, self.topk)

        # 2. Process experts in TP style: every rank holds a shard of every
        # expert, so all ranks evaluate the same experts on the same tokens.
        # Communication (all-reduce) happens inside each ShardedLinear.
        for expert_idx, expert in enumerate(self.experts):
            batch_idx, k_idx = np.where(indices == expert_idx)
            if batch_idx.size == 0:
                continue
            expert_out = expert(x[batch_idx])
            outputs[batch_idx] += gates[batch_idx, k_idx][:, None] * expert_out
...
class MoE_EP:
    ...
    def forward(self, x):
        ...
        # 2. Dispatch tokens: rank j owns expert j. Send each token to the
        # ranks of its top-k experts (all-to-all).
        world_size = mpi.get_size()
        send_tokens = []
        send_meta = []
        for dest in range(world_size):
            batch_idx, k_idx = np.where(indices == dest)
            if batch_idx.size == 0:
                send_tokens.append(np.zeros((0, self.input_dim), dtype=x.dtype))
            else:
                send_tokens.append(np.ascontiguousarray(x[batch_idx]))
            send_meta.append((batch_idx, k_idx))

        recv_tokens = mpi.alltoall(send_tokens)

        # Process the packed local expert inputs, then split back per source rank.
        counts = [tok.shape[0] for tok in recv_tokens]
        if sum(counts) == 0:
            send_outputs = [
                np.zeros((0, self.output_dim), dtype=x.dtype) for _ in recv_tokens
            ]
        else:
            packed = np.concatenate(recv_tokens, axis=0)
            packed_out = self.expert(packed)
            send_outputs = []
            offset = 0
            for count in counts:
                send_outputs.append(packed_out[offset:offset + count])
                offset += count

        # 3. Reverse all-to-all: each rank gets back the expert outputs for
        # the tokens it originally dispatched.
        recv_outputs = mpi.alltoall(send_outputs)

        # 4. Combine gated expert outputs into the full batch result.
        for (batch_idx, k_idx), expert_out in zip(send_meta, recv_outputs):
            if batch_idx.size == 0:
                continue
            outputs[batch_idx] += gates[batch_idx, k_idx][:, None] * expert_out
